...
interface BalloonProps {
  ...
  className?: string;
  // Removed isAutoBurstingActive prop
}
...
const Balloon = forwardRef<any, BalloonProps>(({
  ...
  onBurst,
  className,
  // Removed isAutoBurstingActive from props
}, ref) => {
  ...
  const handleClick = () => {
    // Only allow manual click if it's not already burst.
    if (!internalIsBurst) {
      setInternalIsBurst(true);
      burstBalloon();
    }
    ...
    >
      {!internalIsBurst && (
        <>
          ...
          <div
            ref={balloonRef}
            className="w-24 h-32 md:w-32 md:h-40 rounded-full bg-gradient-to-br from-blue-600 to-purple-800 shadow-lg flex items-center justify-center text-white text-4xl border-2 border-cyan-400"
          ...
          <div
            ref={ropeRef}
            className="w-0.5 h-20 bg-cyan-500 absolute top-full shadow-md shadow-cyan-500/50"
      ...
      {showImage && (
        imageLoadError ? (
          ...
          <img
            ...
            className="absolute w-24 h-24 md:w-32 md:h-32 object-cover rounded-lg shadow-lg border-2 border-cyan-400 z-30"
